...
let adminToken = null;
let authToken = null; // Variable utilisée dans le code existant
...
async function executeChauffeurAction(id, action) {
    ...
    try {
        ...
        // Utiliser l'endpoint admin correct
        const response = await fetch(`${API_BASE}/admin/drivers/${id}/verify`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function executeLivreurAction(id, action) {
    ...
    try {
        ...
        // Utiliser l'endpoint admin correct
        const response = await fetch(`${API_BASE}/admin/delivery-persons/${id}/verify`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function debugLivreurByPhone(telephone) {
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            document.getElementById('modalContent').innerHTML = `
                <div class="space-y-4">
                    <div class="elegant-gradient rounded-lg p-4">
                        <h4 class="font-bold mb-3" style="color: var(--vamo-gold);">👤 INFORMATIONS</h4>
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div><strong>ID:</strong> ${data.livreur.id}</div>
                            <div><strong>Nom:</strong> ${data.livreur.prenom} ${data.livreur.nom}</div>
                            <div><strong>Téléphone:</strong> ${data.livreur.telephone}</div>
                            <div><strong>Type véhicule:</strong> ${data.livreur.type_vehicule || 'Non renseigné'}</div>
                            <div><strong>Statut:</strong> ${data.livreur.statut_validation}</div>
                            <div><strong>Photos trouvées:</strong> ${data.photos_count}/3</div>
                        </div>
                    </div>
                    
                    <div class="elegant-gradient rounded-lg p-4">
                        <h4 class="font-bold mb-3" style="color: var(--vamo-gold);">📸 ANALYSE DES PHOTOS</h4>
                        <div class="space-y-3">
                            ${Object.entries(data.photos_analysis).map(([type, analysis]) => `
                                <div class="flex justify-between items-center p-2 border rounded" 
                                     style="border-color: ${analysis.exists ? '#10B981' : '#EF4444'};">
                                    <div>
                                        <strong>${type.toUpperCase()}</strong>
                                        ${analysis.exists ? 
                                            `<div class="text-xs text-gray-600">Taille: ${analysis.size} caractères</div>` :
                                            `<div class="text-xs text-red-500">Pas de photo</div>`
                                        }
                                    </div>
                                    <div class="text-2xl">
                                        ${analysis.exists ? '✅' : '❌'}
                                ...
                                ${analysis.exists && analysis.preview ? `
                    ...
                    <div class="text-center space-x-4">
                        <button onclick="viewAllPhotos(${data.livreur.id}, 'livreur')" 
                                class="btn-vamo text-white px-6 py-3 rounded-lg font-bold">
                            🖼️ Voir galerie photos
                        </button>
                        <button onclick="closeFuturisticModal()" 
                                class="bg-gray-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-gray-700">
                            🚪 Fermer
